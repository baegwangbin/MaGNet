...
from models.MAGNET import MAGNET_F
...
def train(model, args, device):
    ...
    # define d_centers
    n_depths = 80
    idx_list = np.arange(n_depths + 1)  # (0,1,...,N)
    d_min = args.min_depth
    d_max = args.max_depth
    d_gamma = 1 - d_min
    d_boundaries = np.exp(np.log(d_max + d_gamma) * idx_list / n_depths) - d_gamma  # SID from DORN
    d_left = d_boundaries[:-1]
    d_right = d_boundaries[1:]
    d_center = (d_left + d_right) / 2
    d_center = torch.from_numpy(d_center.astype(np.float32)).view(1, n_depths, 1, 1).to(device)
    ...
    for epoch in range(args.n_epochs):
        ...
        for data_array, cam_intrins in t_loader:
            ...
            # forward pass
            cost_volume = model(ref_img, nghbr_imgs, nghbr_poses, is_valid, cam_intrins, d_center)
            pred_dmap = torch.sum(cost_volume * d_center, dim=1, keepdim=True)

            gt_dmap = F.interpolate(gt_dmap, size=[pred_dmap.shape[2], pred_dmap.shape[3]], mode='nearest')
            gt_dmap_mask = gt_dmap > args.min_depth

            # compute & display loss
            if args.loss_fn == 'l1':
                l1 = torch.abs(pred_dmap[gt_dmap_mask] - gt_dmap[gt_dmap_mask])
                loss = torch.mean(l1)
            else:
                raise Exception
            ...
            if should_write and ((total_iter % args.validate_every) < args.batch_size_orig):
                model.eval()
                metrics = validate(model, args, test_loader, device, d_center)
...
def validate(model, args, test_loader, device, d_center):
    with torch.no_grad():
        metrics = utils.RunningAverageDict()

        for data_array, cam_intrins in tqdm(test_loader, desc=f"Loop: Validation", bar_format='{l_bar}{bar:10}{r_bar}{bar:-10b}'):
            ...
            cost_volume = model(ref_img, nghbr_imgs, nghbr_poses, is_valid, cam_intrins, d_center)
            pred_dmap = torch.sum(cost_volume * d_center, dim=1, keepdim=True)
            pred_dmap = F.interpolate(pred_dmap, size=[ref_img.shape[2], ref_img.shape[3]], mode='nearest')
            ...
            metrics.update(utils.compute_depth_errors(gt_dmap[valid_mask], pred_dmap[valid_mask], None))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--MAGNET_window_radius', type=int, default=20)
